...
static void display(spi_device_handle_t spi);
void calc_lines(uint16_t *dest, int y, int h);

uint8_t current_img = 0;
int64_t now_time, last_time;
...
void app_main() {
    ...
    spi_device_interface_config_t cfg_spi_device = {
        .clock_speed_hz = 10 * 1000 * 1000, // SCLK frequency = 10 MHz
...
void calc_lines(uint16_t *dest, int ypos, int h) {
    ...
    // FIXME: 邏輯不能這樣寫
    if((now_time - last_time) >= 100000) { // 100 [ms]
        printf("delay 100 ms, #img = %d\n", current_img);
    ...
    uint8_t *ptr_img = ptr_img_arr[current_img];
